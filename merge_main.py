...
if __name__ == '__main__':
    ...
    while running:
        ...
        # отображение сердец и анимаций с здоровьем перса, нужен фикс
        heart_image = pygame.image.load('./data/images/interface/healthbar/hp_active/0.png')
        inactive_heart_image = pygame.image.load('./data/images/interface/healthbar/hp_inactive/0.png')
        heart_width, heart_height = heart_image.get_size()

        heart_spacing = 20

        heart_count = player.base_health.base_health.count(1)
        inactive_heart_count = player.base_health.base_health.count(0)

        heart_x = heart_spacing
        heart_y = HEIGHT - heart_height - heart_spacing

        for i in range(heart_count):
            screen.blit(heart_image, (heart_x + i * (heart_width + heart_spacing), heart_y))

        for i in range(inactive_heart_count):

            screen.blit(inactive_heart_image, (heart_x + i * (heart_width + heart_spacing), heart_y))
            images = []
            for j in range(5):
                image_path = './data/images/interface/healthbar/hp_damage/' + str(j) + '.png'
                images.append(pygame.image.load(image_path))
            image_index = 0
            for image in images:
                screen.blit(image, (heart_x + i * (heart_width + heart_spacing), heart_y))
                pygame.display.flip()
                pygame.time.delay(100)  # Задержка между кадрами анимации
                image_index += 1
